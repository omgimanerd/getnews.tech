...
describe('parser.js', () => {
  ...
  describe('parseArgs()', () => {
    ...
    it('should work with valid regular input', () => {
      ...
      expect(fn('p=2')).to.deep.equal({
        p: 2
      })
      expect(fn('n=15')).to.deep.equal({
        n: 15
      })
      expect(fn('query+query2,p=2,n=5')).to.deep.equal({
        query: 'query query2',
        p: 2,
        n: 5
      })
    ...
    it('should throw an error with unparseable input', () => {
      ...
      expect(fnThrow('query,')).to.throw(RecoverableError, reMatch)
      expect(fnThrow(',p=2,')).to.throw(RecoverableError, reMatch)
    })
    ...
    it('should throw an error with invalid parseable input', () => {
      ...
      const reInvalidNaN = /".*" is not a valid value for arg (p|n)\./
      ...
      expect(fnThrow('q,p=2j')).to.throw(RecoverableError, reInvalidNaN)
    })
  })
})
